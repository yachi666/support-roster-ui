...
本文件仅维护根级共用视觉约束与 Public Viewer 设计说明。

Workspace 的视觉语言、管理端布局观感、字体与表面风格统一维护在 `workspace/ui-design.md`，不在本文件重复描述。
...
> 说明：上述原则主要适用于根级共用组件与 Public Viewer。Workspace 作为独立产品域，可在 `workspace/ui-design.md` 中定义不同的视觉节奏与控件密度。

## 浏览器图标

- 应用浏览器 tab 使用 `public/favicon.png` 作为站点图标资源。
- favicon 应优先保证 16x16 与 32x32 场景下的轮廓识别度，不依赖文字。
- 当前图标方向为“对话气泡 + 排班格”，以青蓝霓虹感为主色，强调清爽、年轻和工具型产品气质。
