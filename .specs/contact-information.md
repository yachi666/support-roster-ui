...
- `src/components/Header.vue`：viewer 顶部展示 **Contact Hub** 按钮，并与 **Host Hub** 作为同级工具入口出现。
- `src/features/workspace/components/WorkspaceTopbar.vue`：workspace 顶部展示 **Contact Hub** 按钮，并与 **Host Hub** 共用同一套 secondary action 骨架。
- `src/features/linux-passwords/pages/LinuxPasswordsPage.vue`：Host Hub 顶部展示 **Contact Hub** 按钮，形成跨独立页与主产品域之间的稳定跳转入口。
...
- 跨页入口遵循两层层级：`Contact Hub` 与 `Host Hub` 为同级次级工具入口，`Workspace` / `Open Viewer` 维持更高主动作辨识度。
...
- Team Email 在有值时保留 `mailto:` 跳转；为空时仅保留空单元格，不渲染邮件链接。
- 右上角和页面内容区都可进入新增页。
