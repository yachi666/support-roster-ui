...
- 年份选择器必须为 4 位年份和浏览器原生下拉箭头预留稳定空间；控件宽度不得低于 `6.5rem`，右侧内边距需覆盖原生箭头占位，在 27 寸等大屏或系统字体缩放下年份文本不能被右侧箭头遮挡。
- 右侧动作组应整体包裹成一个独立 action cluster，通过共享白色表面、柔和阴影与统一圆角形成稳定节奏。
- 工具页入口（如 `Host Hub`、`Contact Hub`）保持同级次级气质；跨产品主入口（如 `Open Public Viewer`）保留更高强调度。
...
- Workspace 动效服务于定位、确认、层级变化与状态反馈，不追求展示型页面入场动画。
- 动效分为三档：`micro` 用于 hover/focus 和网格单元格反馈，`standard` 用于状态条、提示与 popover，`panel` 用于 drawer 等大面板进入。
- 大表格和 roster grid 内不允许使用宽泛的 `transition-all` 作为默认策略；应只过渡 `background-color`、`border-color`、`box-shadow` 与必要的 `transform`。
- `prefers-reduced-motion: reduce` 下必须取消位移、缩放与循环动画，只保留即时状态变化。
- 保存成功、保存失败、导入导出错误和待保存状态变化可以使用短 fade/slide 解释结果出现，但不应使用持续 pulse 抢占扫描注意力。
