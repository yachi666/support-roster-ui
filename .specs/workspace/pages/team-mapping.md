...
团队管理页负责维护面向下游展示与排班组织的团队定义，包括名称、颜色、顺序、可见性以及与 role group 的映射关系。
...
| 关联接口 | `GET /api/workspace/role-groups` |
...
- 新建、编辑、删除通过抽屉表单完成，并支持 role group 选择。
- 保存失败时，需要对编码、名称、颜色、顺序、描述和映射字段返回明确错误。
