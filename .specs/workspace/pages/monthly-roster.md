...
- 班次选择动作条应仅通过 `readonly` prop 控制可写状态，页面层不再在事件绑定里重复权限判断。
...
- 抽屉不再提供“指定结束日”的二次输入；网格拖拽到哪一天，就把哪一天作为当前区间填充的结束日。
- 网格可支持同一员工行内拖拽连续日期形成区间选择；松手后应打开抽屉，并让右下角主按钮直接作用于该拖拽范围。
- 当存在拖拽区间选择时，页面或抽屉应提供显式的区间提示与清除入口，避免用户误把区间操作带到下一次编辑。
- 当存在拖拽区间选择时，抽屉保留“重置为单天”入口，并禁用“应用并切到下一天”，避免单天连续编辑语义与区间填充冲突。
...
- `applyShiftCodeToActiveSelection` 应明确接受单格选区 `{ staffId, day }` 或区间选区 `{ staffId, startDay, endDay }`，并在写入前统一归一化。
