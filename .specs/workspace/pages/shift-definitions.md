...
- 当筛选明确锁定到单个 TEAM 时，列表允许拖拽重排该团队下班次定义顺序；即使当前可见班次也共享给其他团队，只要属于所选 TEAM，仍可参与该 TEAM 的排序。
- 页面加载班次定义时始终获取当前权限范围内的完整数据集，不依赖服务端 keyword 过滤；搜索仅影响当前可见的拖拽子集。
- 保存拖拽顺序时仍需提交该 TEAM 的完整班次顺序，并作为 Viewer 中该 TEAM 班次展示顺序的来源。
- 列表在单个 TEAM 筛选下需要按接口返回的 `teams[].displayOrder` 本地重排，而不是继续复用全局 `code` 排序，否则共享班次无法稳定回显 TEAM 维度顺序。
- 拖拽保存时必须保留 `selectedTeamFilter` 的字符串主键原值；不要把 Snowflake 风格 TEAM id 强转成 `Number`，否则会因精度丢失导致重排接口命中错误团队。

## 数据边界与约束

- 班次不仅影响月排班可选项，也影响导入校验和 tooltip 展示。
- 团队关联使用多选结构，而不是单团队字段。
- `teams[].displayOrder` 属于 TEAM 维度元数据，不改变主显示团队，但决定当前 TEAM 下的拖拽顺序回显。
