...
- `Host Hub` 与 `Contact Hub` 属于同类工具页入口，在 Topbar 中应作为同级次级按钮成组出现，而不是与主动作混成一排无层级的链接。
...
- 月份上下文区需要支持渐进压缩：月份摘要持续可见，月份/年份选择器可压缩但必须保留完整可读文本，年份选择器不得低于 `6.5rem`，时区与语言控件收缩为更紧凑的 meta control。
...
- 壳层级 popover、状态条和覆盖层应复用 Workspace motion 规则，避免每个页面定义一套独立时长与曲线。
- Drawer 与 Modal 的遮罩、面板进入和退出应保持同一层级语言；减少动效时必须保留结构和状态的即时可见性。
