...
本文件聚焦构建产物、容器封装、Fargate 运行边界与发布规则，属于手册中的“部署分册”。阅读时建议先结合 `spec.md` 与 `architecture.md` 建立全局背景，再回到这里确认上线约束。

## 阅读提示

- 重点关注构建期变量注入、容器职责边界和 SPA 路由回退要求。
- 若部署链路、镜像封装或健康检查策略变化，优先回写本文件。
