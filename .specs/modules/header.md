...
Header 是顶部导航组件，提供日期选择、UTC/HKT/IST 时区切换、实时时钟和跨产品页面入口。采用 `v-model` 双向绑定模式与父组件通信。
...
- Header 右侧工具区依次展示 Contact Information、Linux Password Vault、Product Updates 和 Workspace。
- Contact Information 与 Linux Password Vault 使用文字 + 图标按钮，保持普通工具入口层级。
- Product Updates 使用仅图标按钮，`to="/product-updates"`，图标采用 `Newspaper`，用于快速打开产品更新日志中心。
...
import { Calendar, Clock, ContactRound, Globe, KeyRound, LayoutDashboard, Newspaper } from 'lucide-vue-next'
```
