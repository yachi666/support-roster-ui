...
- **Implemented**: 固定宽度时间轴 (`HOUR_WIDTH = 120`)、24h 横向滚动、重叠班次自动分泳道、Tooltip 详情。
...
```javascript
const HOUR_WIDTH = 120        // 每小时像素宽度
const TOTAL_WIDTH = 24 * HOUR_WIDTH  // 时间轴总宽度 (2880px)
const BLOCK_HEIGHT = 36       // 班次卡片高度
const BLOCK_GAP = 8           // 卡片间距
const ROW_PADDING = 16        // 行内边距
const MIN_ROW_HEIGHT = 80     // 最小行高
```
...
for each shift in sorted_shifts:
    ...
    // 计算像素位置
    left = (visibleStart - viewWindow.start) / 1hour * HOUR_WIDTH
    width = (visibleEnd - visibleStart) / 1hour * HOUR_WIDTH
  ...
  <div class="h-full overflow-auto bg-white relative">
    <div class="min-w-max flex flex-col">
      ...
      <div class="sticky top-0 z-30 flex bg-gray-50 border-b">
        <!-- Team 列标题 -->
        <div class="sticky left-0 w-60">Teams</div>
        
        <!-- 小时刻度 -->
        <div class="flex" :style="{ width: TOTAL_WIDTH }">
          <div v-for="h in 24" class="w-[120px]">{{ h }}:00</div>
      ...
      <div class="relative">
        ...
        <div v-for="{ team, height, shifts } in layout" class="flex border-b">
          ...
          <!-- 班次卡片区 -->
          <div class="relative" :style="{ width: TOTAL_WIDTH, height }">
            <!-- 空状态背景 -->
