...
graph TB
    ...
    ROUTER --> VIEWER
    ROUTER --> CONTACT
    ROUTER --> PRODUCTUPDATES
    ROUTER --> WORKSPACE
    VIEWER --> PAGE1
    CONTACT --> PAGEC
    PRODUCTUPDATES --> PAGEU
    PAGE1 --> DASH
    DASH --> API
    PAGE2 --> API
    API --> SERVER
...
| `/product-updates` | 产品更新日志 | `src/features/product-updates/pages/ProductUpdatesPage.vue` | 独立全屏、静态种子数据、展示当前版本和版本详情 |
...
| 页面层 | `src/pages/`、`src/features/contact-information/pages/`、`src/features/product-updates/pages/`、`src/features/workspace/pages/` | 组织页面状态与交互流程 |
| 组件层 | `src/components/`、`src/features/contact-information/components/`、`src/features/product-updates/components/`、`src/features/workspace/components/` | 渲染结构、承载局部交互 |
...
- 面向 `/product-updates`，强调产品版本变化的回溯、当前版本识别和详情阅读。
- 采用独立布局，不复用 `WorkspaceLayout` 或 Public Viewer 的组件。
- 当前数据来源为本地静态种子数据，后续可替换为产品更新日志 API。
...
- Product Updates 以页面局部状态和 `computed` 派生为主，不引入全局 store。
