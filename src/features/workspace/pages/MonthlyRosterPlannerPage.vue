...
function handleGridNavigation({ rowDelta, dayDelta, staffId, day }) {
  ...
  const currentIndex = rows.findIndex(
    (row) => row.staffId === (staffId || selectedCell.value?.staffId),
  )
  const safeIndex = currentIndex === -1 ? 0 : currentIndex
  const nextIndex = Math.min(Math.max(safeIndex + rowDelta, 0), rows.length - 1)
  const nextDay = Math.min(
    Math.max((day || selectedCell.value?.day || 1) + dayDelta, 1),
    plannerDays.value.length,
  )
...
<template>
  <div class="relative flex h-full flex-col bg-white">
    <div
      class="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 shadow-sm"
    >
      <div class="flex items-center gap-4">
        <div>
          <div class="text-xs font-semibold uppercase tracking-wide text-slate-400">
            {{ t('workspace.roster.title') }}
          </div>
          <div class="flex items-center gap-2 text-sm font-semibold text-slate-800">
            <span>{{ monthLabel }}</span>
            <span
              v-if="hasUnsavedChanges"
              class="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] uppercase tracking-wide text-amber-700"
            >
          ...
          >
            ...
            {{
              selectedTeamIds.length > 0
                ? t('workspace.roster.teamButton', { count: selectedTeamIds.length })
                : t('workspace.roster.teamButtonAll')
            }}
          </button>
          <Transition name="workspace-popover">
            <div
              v-if="showTeamFilter"
              class="absolute right-0 top-full z-50 mt-2 w-64 origin-top-right rounded-lg border border-slate-200 bg-white p-3 shadow-lg"
            >
              <div class="flex items-center justify-between mb-2">
                <span class="text-xs font-semibold uppercase tracking-wide text-slate-500">{{
                  t('workspace.roster.filterByTeam')
                }}</span>
                <button
    ...
    <WorkspaceSurface
      v-if="errorMessage"
      tone="muted"
      class="m-4 border-rose-200 bg-rose-50 p-5 text-sm text-rose-700"
    >
    ...
    <Transition name="workspace-status">
      <div
        ...
        class="mx-4 mt-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700"
      >
        {{ saveSuccessMessage }}
      </div>
    </Transition>
    ...
    <Transition name="workspace-status">
      <div
        v-if="importExportError"
        class="mx-4 mt-4 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700"
      >
        <div class="flex items-center justify-between gap-4">
          <span>{{ importExportError }}</span>
          <button
            class="rounded-md border border-rose-200 bg-white px-3 py-1.5 text-xs font-medium text-rose-700 transition-colors hover:bg-rose-100"
            @click="importExportError = ''"
          >
