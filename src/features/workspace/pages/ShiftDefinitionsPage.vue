...
const visibleShifts = computed(() => {
  ...
  return shiftDefinitions.value.filter((shift) => {
    ...
    const matchesSearch =
      !query ||
      [shift.code, shift.meaning, shift.teamName, teamNames].join(' ').toLowerCase().includes(query)
...
const canEditSelectedShift = computed(
  () => Boolean(selectedShift.value) && authStore.canEditAllTeams(selectedShiftTeamIds.value),
)
...
const drawerTitle = computed(() => {
  ...
  return isReadOnlyDrawer.value
    ? t('workspace.shifts.readonlyTitle')
    : t('workspace.shifts.editTitle')
})
...
const timeSummary = computed(() =>
  describeShiftWindow(formState.startTime, formState.durationMinutes),
)
...
function fillForm(shift) {
  const shiftTeams = shift?.teams?.length
    ? shift.teams
    : shift?.teamId
      ? [{ id: shift.teamId }]
      : []
  ...
  Object.assign(formState, {
    ...
    durationMinutes:
      calculateShiftWindow(shift?.startTime, shift?.durationMinutes ?? shift?.endTime)
        .durationMinutes || 8 * 60,
    ...
    colorHex: normalizeHexColor(shift?.colorHex),
    remark: shift?.remark || '',
  })
}
...
function validateForm() {
  ...
  if (!/^[A-Za-z0-9_+-]{1,16}$/.test(formState.code.trim()))
    fieldErrors.code = 'Use 1-16 letters, numbers, _, + or -.'
  ...
  if (!normalizeDurationMinutes(formState.durationMinutes))
    fieldErrors.durationMinutes = 'Duration is required.'
  if (normalizeDurationMinutes(formState.durationMinutes) > 1440)
    fieldErrors.durationMinutes = 'Duration must be 1440 minutes or fewer.'
  if (normalizeDurationMinutes(formState.durationMinutes) % 30 !== 0)
    fieldErrors.durationMinutes = 'Duration must use 30-minute steps.'
  if (formState.colorHex.trim() && !/^#([0-9a-fA-F]{6})$/.test(formState.colorHex.trim()))
    fieldErrors.colorHex = 'Use a 6-digit hex color.'
  if (!formState.timezone.trim()) fieldErrors.timezone = 'Timezone is required.'
  if (formState.timezone.trim().length > 64)
    fieldErrors.timezone = 'Timezone must be 64 characters or fewer.'
  if (formState.remark.trim().length > 500)
    fieldErrors.remark = 'Remark must be 500 characters or fewer.'
...
async function saveShift() {
  ...
  try {
    ...
    formErrorMessage.value = hasFieldErrors
      ...
      : getApiErrorMessage(error, 'Failed to save shift definition.')
  } finally {
  ...
  <div class="flex h-full flex-col bg-slate-50">
    <div class="flex-1 overflow-auto p-8">
      <div class="mx-auto flex max-w-7xl flex-col gap-8">
        ...
        >
          <template #actions>
            <div class="relative">
              <label for="workspace-shift-search" class="sr-only">{{
                t('workspace.shifts.searchLabel')
              }}</label>
              <Search
            ...
            <button
              v-if="canCreateShift"
              class="flex items-center gap-2 rounded-md bg-teal-600 px-4 py-2 text-sm font-medium text-white shadow-sm transition-colors hover:bg-teal-700"
              ...
              <tbody class="divide-y divide-slate-100">
                ...
                >
                  ...
                  <td class="px-6 py-4">
                    <div v-if="getShiftTeams(shift).length" class="flex flex-wrap gap-1.5">
                      <span
                        v-for="team in getShiftTeams(shift)"
                        :key="team.id"
                  ...
                  <td class="px-6 py-4">
                    <div class="flex items-center gap-2 font-mono text-xs text-slate-600">
                      ...
                      <span>{{
                        describeShiftWindow(shift.startTime, shift.durationMinutes ?? shift.endTime)
                      }}</span>
                      <span class="rounded bg-slate-100 px-1 text-[10px] text-slate-400">{{
                        normalizeWorkspaceStaffTimezone(shift.timezone)
                      }}</span>
                    </div>
                  </td>
                  <td class="px-6 py-4">
                    <div
                      class="relative h-1.5 w-full overflow-hidden rounded-full border border-slate-200 bg-slate-100"
                    >
                      <div
                        v-for="(segment, index) in previewSegments(shift)"
                        :key="`${shift.id || shift.code}-segment-${index}`"
                  ...
                  <td class="px-6 py-4 text-center">
                    ...
                    >
                      {{
                        shift.primaryShift
                          ? t('workspace.shifts.primary')
                          : t('workspace.shifts.secondary')
                      }}
                    </span>
                  </td>
                  <td class="px-6 py-4 text-center">
                ...
                <tr v-if="loading">
                  <td colspan="8" class="px-6 py-10 text-center text-sm text-slate-500">
                    {{ t('workspace.shifts.loading') }}
                  </td>
                </tr>
                <tr v-else-if="!visibleShifts.length">
                  <td colspan="8" class="px-6 py-10 text-center text-sm text-slate-500">
                    {{ t('workspace.shifts.empty') }}
                  </td>
                </tr>
              </tbody>
            </table>
    ...
    >
      ...
      <form class="space-y-5" @submit.prevent="saveShift">
        ...
        <WorkspaceSurface
          v-if="isReadOnlyDrawer"
          tone="muted"
          class="border-slate-200 bg-slate-50 p-4 text-sm text-slate-700"
        >
        ...
        <WorkspaceSurface
          v-if="confirmDeleteVisible && canEditSelectedShift"
          tone="muted"
        ...
        <div class="grid gap-4 md:grid-cols-2">
          <label class="space-y-2 text-sm text-slate-700 md:col-span-2">
            ...
            <div
              :class="[
                ...
                fieldErrors.teamIds ? 'border-rose-300' : 'border-slate-200',
              ]"
            >
              <div class="mb-2 flex items-center justify-between text-xs text-slate-500">
                <span>{{ t('workspace.shifts.teamHint') }}</span>
                <span>{{
                  t('workspace.shifts.selectedCount', { count: formState.teamIds.length })
                }}</span>
              </div>
              <div class="grid gap-2 md:grid-cols-2">
                <label
                  ...
                  :class="[
                    ...
                    isReadOnlyDrawer
                      ? 'cursor-not-allowed bg-slate-50/70 opacity-80'
                      : 'cursor-pointer hover:border-teal-300 hover:bg-teal-50/40',
                  ]"
                >
                  <input
                    :checked="formState.teamIds.includes(String(team.id))"
                    :disabled="isReadOnlyDrawer"
                    type="checkbox"
                    class="h-4 w-4 rounded border-slate-300 text-teal-600 focus:ring-teal-500"
                    @change="toggleFormTeam(team.id)"
                  />
                  <span
                    class="h-2.5 w-2.5 rounded-full"
                    :style="{ backgroundColor: resolveWorkspaceColor(team.color, '#94a3b8') }"
                  ></span>
                  <span>{{ team.name }}</span>
                </label>
              </div>
            </div>
            <p v-if="fieldErrors.teamIds" class="text-xs text-rose-600">
              {{ fieldErrors.teamIds }}
            </p>
          </label>
          ...
          <label class="space-y-2 text-sm text-slate-700">
            ...
            <input
              id="shift-meaning"
              v-model="formState.meaning"
              :disabled="isReadOnlyDrawer"
              name="meaning"
              type="text"
              :class="inputClass('meaning')"
            />
            <p v-if="fieldErrors.meaning" class="text-xs text-rose-600">
              {{ fieldErrors.meaning }}
            </p>
          </label>
          <label class="space-y-2 text-sm text-slate-700">
            ...
            <select
              ...
              :disabled="isReadOnlyDrawer"
              name="startTime"
              :class="['bg-white', ...inputClass('startTime')]"
              @change="handleStartTimeChange"
            >
              <option
                v-for="timeOption in TIME_OPTIONS"
            ...
            <p v-if="fieldErrors.startTime" class="text-xs text-rose-600">
          ...
          <label class="space-y-2 text-sm text-slate-700">
            ...
            />
            <p v-if="fieldErrors.durationMinutes" class="text-xs text-rose-600">
          ...
          <div class="space-y-2 text-sm text-slate-700 md:col-span-2">
            ...
            <div class="flex flex-wrap gap-2">
              <button
                type="button"
                :disabled="isReadOnlyDrawer"
                class="rounded-md border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 transition-colors hover:border-teal-300 hover:bg-teal-50 hover:text-teal-700 disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400"
                @click="applyDurationPreset(8 * 60)"
              >
                +8h
              </button>
              <button
                ...
                :disabled="isReadOnlyDrawer"
                class="rounded-md border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 transition-colors hover:border-teal-300 hover:bg-teal-50 hover:text-teal-700 disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400"
                @click="applyDurationPreset(9 * 60)"
              >
                +9h
              </button>
              <button
                ...
                :disabled="isReadOnlyDrawer"
                class="rounded-md border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 transition-colors hover:border-teal-300 hover:bg-teal-50 hover:text-teal-700 disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400"
                @click="applyDurationPreset(12 * 60)"
              >
                +12h
              </button>
              <button
                ...
                :disabled="isReadOnlyDrawer"
                class="rounded-md border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 transition-colors hover:border-teal-300 hover:bg-teal-50 hover:text-teal-700 disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400"
                @click="applyDurationPreset(24 * 60)"
              >
                +24h
              </button>
            </div>
            <p class="text-xs text-slate-500">
              {{ timeSummary || t('workspace.shifts.selectStartAndDuration') }}
            </p>
            <p v-if="endTimeValue" class="text-xs text-slate-400">
              {{ t('workspace.shifts.endsAt', { time: endTimeValue, duration: durationLabel }) }}
            </p>
          </div>
          <label class="space-y-2 text-sm text-slate-700">
            ...
            <select
              ...
              :disabled="isReadOnlyDrawer"
              name="timezone"
              :class="['bg-white', ...inputClass('timezone')]"
            >
              <option
                v-for="timezoneOption in WORKSPACE_STAFF_TIMEZONE_OPTIONS"
            ...
            <p v-if="fieldErrors.timezone" class="text-xs text-rose-600">
          ...
          <div class="space-y-2">
            <label
              class="flex items-center gap-3 rounded-lg border border-slate-200 px-3 py-3 text-sm text-slate-700"
            >
              <input
                id="shift-primaryShift"
                v-model="formState.primaryShift"
                :disabled="isReadOnlyDrawer"
                name="primaryShift"
                type="checkbox"
                class="h-4 w-4 rounded border-slate-300 text-teal-600 focus:ring-teal-500 disabled:cursor-not-allowed"
              />
      ...
      <template #footer>
        <div class="flex items-center justify-between gap-3">
          ...
          <div class="flex items-center gap-3">
            ...
            <button
              v-if="canCreateShift && !isReadOnlyDrawer"
              type="button"
              class="inline-flex items-center gap-2 rounded-md bg-teal-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-teal-700 disabled:cursor-not-allowed disabled:opacity-60"
            ...
            >
              ...
              {{
                submitPending
                  ? t('common.saving')
                  : selectedShift
                    ? t('workspace.shifts.saveAction')
                    : t('workspace.shifts.createShift')
              }}
            </button>
          </div>
        </div>
