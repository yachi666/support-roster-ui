...
test('workspace topbar action links share one button skeleton while keeping separate visual variants', () => {
  assert.match(
    source,
    /const topbarActionLinkBaseClass =\s*'inline-flex h-9 shrink-0 items-center justify-center gap-2 whitespace-nowrap rounded-full px-3\.5 text-sm font-medium transition-colors'/,
  )
  assert.match(
    source,
    /const topbarActionSecondaryClass =\s*'border border-slate-200 bg-white text-slate-700 hover:border-slate-300 hover:bg-slate-50'/,
  )
  assert.match(
    source,
    /const topbarActionPrimaryClass =\s*'border border-teal-200 bg-teal-50 text-teal-700 hover:border-teal-300 hover:bg-teal-100'/,
  )
...
test('workspace topbar separates search, period context, and action cluster into distinct surfaces', () => {
  assert.match(
    source,
    /const topbarSearchShellClass =\s*'group relative w-\[9\.5rem\] shrink min-\[1280px\]:w-\[11rem\] min-\[1536px\]:w-\[12rem\]'/,
  )
  assert.match(
    source,
    /const topbarPeriodShellClass =\s*'flex h-9 w-fit shrink-0 items-center justify-center gap-0\.5 rounded-xl border border-slate-200 bg-white px-1 shadow-sm'/,
  )
  assert.match(
    source,
...
test('workspace topbar uses one date picker surface instead of duplicating month display', () => {
  ...
  assert.match(
    source,
    /const topbarMonthSelectClass =\s*'h-7 w-\[4\.75rem\] rounded-lg border border-transparent bg-slate-50 pl-2 pr-6 text-sm font-semibold text-slate-900 outline-none transition-colors hover:border-slate-200 focus:border-teal-300 focus:bg-white focus:ring-2 focus:ring-teal-500\/15'/,
  )
  assert.match(
    source,
    /const topbarMetaControlClass =\s*'flex h-7 min-w-0 items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 pl-1\.5 pr-1'/,
  )
})
...
test('workspace topbar stays as a single compact toolbar instead of wrapping into multiple rows', () => {
  ...
  assert.match(
    source,
    /<header\s+class="flex flex-shrink-0 overflow-x-auto border-b border-slate-200\/80 bg-slate-50\/90 px-5 backdrop-blur-sm lg:px-6"/,
  )
