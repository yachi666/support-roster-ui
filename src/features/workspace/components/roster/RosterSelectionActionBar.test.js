...
test('selection action bar exposes shift code options and emits code selection', () => {
  assert.match(source, /defineProps\(\{[\s\S]*visible:[\s\S]*readonly:[\s\S]*shiftCodeOptions:/)
  assert.match(source, /emit\('select-code', code\)/)
  assert.match(source, /v-for="code in actionShiftCodes"/)
  assert.match(source, /emit\('select-code', 'Clear'\)/)
