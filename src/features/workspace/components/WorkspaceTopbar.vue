...
const topbarSearchShellClass =
  'group relative w-[9.5rem] shrink min-[1280px]:w-[11rem] min-[1536px]:w-[12rem]'
const topbarSearchInputClass =
  'h-9 w-full rounded-lg border border-slate-200 bg-white py-2 pl-9 pr-2 text-sm text-slate-700 shadow-sm outline-none transition-all placeholder:text-slate-400 focus:border-teal-500 focus:ring-2 focus:ring-teal-500/15'
const topbarPeriodShellClass =
  'flex h-9 w-fit shrink-0 items-center justify-center gap-0.5 rounded-xl border border-slate-200 bg-white px-1 shadow-sm'
...
const topbarActionLinkBaseClass =
  'inline-flex h-9 shrink-0 items-center justify-center gap-2 whitespace-nowrap rounded-full px-3.5 text-sm font-medium transition-colors'
const topbarActionSecondaryClass =
  'border border-slate-200 bg-white text-slate-700 hover:border-slate-300 hover:bg-slate-50'
const topbarActionPrimaryClass =
...
<template>
  <header
    class="flex flex-shrink-0 overflow-x-auto border-b border-slate-200/80 bg-slate-50/90 px-5 backdrop-blur-sm lg:px-6"
  >
    <div :class="topbarShellClass">
      <div :class="topbarSearchShellClass">
        <label for="workspace-topbar-search" class="sr-only">{{
          t('workspace.shell.topbar.searchLabel')
        }}</label>
        <Search
      ...
      <div :class="topbarPeriodShellClass">
        ...
        <label for="workspace-topbar-month" class="sr-only">{{
          t('workspace.shell.topbar.selectMonth')
        }}</label>
        <select
        ...
        <label for="workspace-topbar-year" class="sr-only">{{
          t('workspace.shell.topbar.selectYear')
        }}</label>
        <select
        ...
        <label for="workspace-topbar-timezone" class="sr-only">{{
          t('workspace.shell.topbar.selectTimezone')
        }}</label>
        <div :class="topbarMetaControlClass">
          ...
          >
            <option
              v-for="timezoneOption in WORKSPACE_TIMEZONE_OPTIONS"
